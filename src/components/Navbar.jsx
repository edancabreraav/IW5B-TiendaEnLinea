...
const Navbar = () => {
  ...
  return (
    <div className="navbar">
      ...
      <Link to="/" className="navbar-button" title="Inicio"><HomeIcon/></Link>
      ...
      {user.username === null ? (
        ...
        <div>
          <Link to='/'><button
            className="navbar-button"
            title="Cerrar Sesión"
            onClick={() => {setUser({ email: null, username: null, role: null }); setIsOpen(false); setCart([]); alert(`¡Vuelve pronto ${user.username}!`);} }
          >
            <LogoutIcon/>
          </button></Link>
          {user.role === "client" && (
            <button className="navbar-button" title="Ver carrito" onClick={toggleCart}>
              <ShoppingCartIcon/>
            </button>
          )}
